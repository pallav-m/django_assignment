...
class TestVendorFetchAPI(APITestCase):
    """
    Test suite to delete a purchase order.
    """
    ...
    def test_po_delete_api(self):
        response = self.client.delete('/api/purchase_orders/1/')

        self.assertEquals(response.status_code, status.HTTP_200_OK)
        self.assertEquals(response.data, {'response': 'PO deleted successfully.'})

    def test_po_delete_api_without_authorization(self):
        self.client.credentials(HTTP_AUTHORIZATION='')
        response = self.client.delete(f'/api/purchase_orders/1/')

        self.assertEquals(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_po_delete_api_if_vendor_does_not_exist(self):
        response = self.client.delete(f'/api/purchase_orders/5/')
